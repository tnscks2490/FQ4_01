...
//  이 검색방법은 대상 노드와 현재 노드 사이의 거리를 사용하여 그래프를 검색합니다. 
//  현재 노드는 휴리스틱으로 간주됩니다.?
//
//  This search is more commonly known as A* (pronounced Ay-Star)
//  이 검색은 일반적으로 AStar로 알려져 있다.
//-----------------------------------------------------------------------------
...
private:
  ...
  //create a typedef for the edge type used by the graph
  //그래프에서 사용되는 엣지(노드 사이의 길) 유형에 대한 typedef를 만듭니다.
  typedef typename graph_type::EdgeType Edge;
...
private:
  ...
  //내 노드에 인덱스를 가리킨다. 해당 노드에 대한 '실제' 누적 비용을 포함합니다.
  ...
  //iPQ indexes into.
  //노드별로 인덱싱됩니다. n에서 대상 노드까지의 경험적 비용에
  //m_Gcosts[n]을 추가하는 데 따른 비용이 포함됩니다. 
  //이는 iPQ가 색인화하는 벡터입니다.
...
void Graph_SearchAStar<graph_type, heuristic>::Search()
{
  ...
  //노드의 색인화(인덱싱)된 우선순위 큐를 생성합니다.
  //노드는 가장 낮은 전체 F 비용(G + H)이 전면에 배치됩니다.
  IndexedPriorityQLow<double> pq(m_FCosts, m_Graph.NumNodes());
  // -> 우선 순위 큐 생성하는 함수

  //put the source node on the queue
  //소스 노드(m_iSource)를 대기열에 넣는다.
  pq.insert(m_iSource);

  //while the queue is not empty
  //큐 안에 정보가 존재하면 계속 반복한다.(큐 안에 아무것도 없으면 break)
  while(!pq.empty())
  {
    //get lowest cost node from the queue
    //대기열에서 가장 낮은 비용의 노드를 가져온다.
    int NextClosestNode = pq.Pop();

    //move this node from the frontier to the spanning tree
    //이 노드를 프론티어에서 스패닝 트리로 이동한다.
    m_ShortestPathTree[NextClosestNode] = m_SearchFrontier[NextClosestNode];
