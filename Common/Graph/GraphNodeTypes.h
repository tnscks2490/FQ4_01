...
class NavPosition : public GraphNode
{
protected:
...
public:
  ...
  //ctors
  NavPosition():m_ExtraInfo(extra_info()){}
  ...
  //stream constructor
  NavPosition(std::ifstream& stream):m_ExtraInfo(extra_info())
  {
    char buffer[50];
    ...
    stream >> m_iIndex;
    stream >> buffer;
    stream >> m_vPosition.x;
    stream >> buffer;
    stream >> m_vPosition.y;
  ...
  //for reading and writing to streams.
  friend std::ostream& operator<<(std::ostream& os, const NavPosition& n)
  {
